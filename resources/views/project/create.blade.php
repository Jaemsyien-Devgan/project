...
<x-app-layout>
    <div class="min-h-screen bg-[#2f4f4f] flex flex-col justify-center items-center p-8">
        ...
        <div class="bg-white text-green-800 w-full max-w-6xl rounded-lg shadow-lg p-6 grid grid-cols-1 ">
            ...
            <div>
                <form action="{{ route('project.store') }}" class="space-y-6 col-span-1" method="post" enctype="multipart/form-data">
                @csrf
                <!-- Judul Project -->
                <div>
                    <label for="judul" class="block text-gray-700 font-extrabold text-base">JUDUL PROJECT</label>
                    <input type="text" name="judul" id="judul" class="font-bold bg-white w-full p-3 border border-2 border-green-300 rounded-lg focus:outline-none focus:border-green-300" placeholder="Masukkan Judul Project">
                    @error('judul')
                ...
                <div>
                    ...
                    <textarea name="deskripsi" name="deskripsi" id="deskripsi" cols="30" rows="10" class="text-gray-700 font-bold bg-white w-full h-40 p-3 border border-2 border-green-300 rounded-lg focus:outline-none focus:border-green-300" placeholder="Masukkan Deskripsi Project"></textarea>
                    @error('deskripsi')
                    <div class="text-red-500">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="file" id="image" name="image" class="text-gray-700 font-bold bg-white w-full p-3 border border-2 border-green-300 rounded-lg focus:outline-none focus:border-green-300">
                    @error('image')
                    <div class="text-red-500">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="penulis" name="penulis" class="text-gray-700 font-bold bg-white w-full p-3 border border-2 border-green-300 rounded-lg focus:outline-none focus:border-green-300" placeholder="Masukkan Penulis Project">
                    @error('penulis')
                    <div class="text-red-500">{{ $message }}</div>
                    @enderror
                </div>
